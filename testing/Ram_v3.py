...
class AnnotationItem(QGraphicsItem):
    ...
    Uses ItemIgnoresTransformations so the badge and label are ALWAYS the
    same size on screen regardless of zoom level — identical to how CVAT,
    Roboflow, and Label Studio render annotation overlays.

    Position is set via setPos(cx, cy) in scene coordinates so the badge
    stays anchored to the circle centre through all pan/zoom operations.

    ALL geometry in paint() is in local screen-pixel coordinates (relative
    to the circle centre).  This means:
      • Badge   : 16 px radius circle at local (0, 0)
      • Label   : fixed offset to the right, centred vertically on badge
      • No scale-dependent coordinates anywhere — no race with zoom changes
    """

    Z_VALUE  = 20
    DOT_R    = 7      # screen pixels — small anchor dot (no number inside)
    FONT_PX  = 18     # screen pixels — medium readable label font
    PAD      = 8      # screen pixels — label box inner padding
    ...
    def __init__(
        ...
        img_w:        int,
        img_h:        int,
        placed_rects: list,   # kept for API compat; not used (screen coords)
    ):
        super().__init__()
        self.setZValue(self.Z_VALUE)
        # ItemIgnoresTransformations: item moves with scene but never scales
        self.setFlag(
        ...
        self.setPos(cx, cy)   # anchor to scene coords; all paint() is local

        col = _MODE_QT[circle["mode"]]
        lbl = circle["label"] or f"Object #{index}"
        tag = f"#{index}  {lbl}"

        # ── Pre-bake font (never allocate in paint) ───────────────────────
        self._font_label = QFont()
        self._font_label.setPixelSize(self.FONT_PX)
        self._font_label.setBold(True)

        fm = QFontMetrics(self._font_label)
        tw = fm.horizontalAdvance(tag)
        th = fm.height()

        # Label box: immediately to the right of dot (screen coords)
        gap  = 10                             # gap between dot edge and label
        ox   = self.DOT_R + gap + 6          # +6 = width of accent bar
        lw   = tw + 2 * self.PAD + 6
        lh   = th + 2 * self.PAD
        oy   = -(lh // 2)                    # vertically centred on dot

        self._col   = col
        self._tag   = tag
        self._lrect = QRectF(ox, oy, lw, lh)
        self._ltx   = ox + 10                # text origin x (after accent)

    # ── save_current still uses _find_label_pos for OpenCV label layout ───
    ...
    def _find_label_pos(
        ...
        """
        Non-overlapping label position for the SAVED IMAGE (OpenCV draw).
        Returns (text_x, text_y, rect_tuple).
        Not used by the Qt overlay — kept for save_current compatibility.
        """
        candidates = [
            (cx + r + 10,      cy - H / 2),
            (cx - r - W - 10,  cy - H / 2),
        ...
        extras = []
        for off in (50, 100, 160):
            extras += [(px, py + off) for px, py in candidates[:4]]
            extras += [(px, py - off) for px, py in candidates[:4]]
        candidates += extras
        ...
        for px, py in candidates:
            ...
            if (rect[0] < pad or rect[1] < pad or
                    rect[2] > iw - pad or rect[3] > ih - pad):
                continue
            if not any(_rects_overlap(rect, r2) for r2 in placed):
                return px + pad, py + H - pad, rect

        # Fallback: stack below all placed labels
        bottom = max((r2[3] for r2 in placed), default=cy + r) + 8
        lx     = max(pad, min(cx - W / 2, iw - W - pad))
        rect   = (lx, bottom, lx + W, bottom + H)
        return lx + pad, bottom + H - pad, rect

    # ── QGraphicsItem interface ────────────────────────────────────────────

    def boundingRect(self) -> QRectF:
        dr = self.DOT_R + 3
        dot_r = QRectF(-dr, -dr, 2 * dr, 2 * dr)
        return dot_r.united(self._lrect).adjusted(-4, -4, 4, 4)
    ...
    def paint(self, painter: QPainter, option, widget=None):
        ...
        # ── Connector line: dot-edge → label left-edge ─────────────────────
        lx1 = self._lrect.x()
        lcy = self._lrect.center().y()
        painter.setOpacity(0.80)
        pen_c = QPen(col, 1.8)
        pen_c.setCosmetic(True)
        painter.setPen(pen_c)
        painter.setBrush(Qt.BrushStyle.NoBrush)
        painter.drawLine(QPointF(dr, 0.0), QPointF(lx1, lcy))

        # ── Small anchor dot at circle centre ──────────────────────────────
        # White outer ring for visibility on any background colour
        painter.setOpacity(1.0)
        painter.setBrush(QBrush(QColor(255, 255, 255)))
        painter.setPen(Qt.PenStyle.NoPen)
        painter.drawEllipse(QPointF(0.0, 0.0), dr + 2, dr + 2)
        # Coloured inner fill
        painter.setBrush(QBrush(col))
        painter.drawEllipse(QPointF(0.0, 0.0), dr, dr)
        ...
        painter.setBrush(QBrush(col))
        painter.setPen(Qt.PenStyle.NoPen)
        painter.drawRoundedRect(QRectF(x1, y1, 5, y2 - y1), 2, 2)
        ...
        inner = QRectF(x1 + 10, y1, x2 - x1 - 14, y2 - y1)
        painter.drawText(
            inner,
...
class ImageViewer(QGraphicsView):
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        if self._panning and self._pan_origin is not None:
            ...
            # Throttle: skip ghost update if < 16 ms since last redraw
            import time
...
class MainWindow(QMainWindow):
    ...
    def _apply_circle_to_effects(self, c: dict):
        ...
        Before modifying pixels, saves a ROI snapshot (tiny bounding-box copy)
        into c["_undo_roi"] so that undo can restore just those pixels in O(r²)
        without rebuilding all n-1 effects from scratch.
        """
    ...
    def _rebuild_all_annotation_items(self):
        ...
        for i, c in enumerate(self._circles, 1):
            item = AnnotationItem(i, c, self._display_scale,
                                  dw, dh, self._placed_rects)
    ...
    def _push_new_annotation_item(self, c: dict):
        ...
        item   = AnnotationItem(index, c, self._display_scale,
                                dw, dh, self._placed_rects)
        item.setVisible(self._show_labels)
        self._viewer.add_annotation_item(item)
...
""")
    ...
    def _navigate(self, delta: int):
        ...
        if self._circles:
            self._cache_circles()
            # Always auto-save when leaving — covers newly added circles
            # even if the image was previously saved once before
    ...
    def save_current(self, auto_save: bool = False):
        """
        Reload source file at FULL resolution, apply effects + labels
        at original coords, write output + JSON, free immediately.
        Peak RAM ≈ 2× raw file size for < 1 second.
        """
        if not self._circles:
            if not auto_save:
