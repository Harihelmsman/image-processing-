...
class AnnotationItem(QGraphicsItem):
    ...
    def _find_label_pos(
        ...
        """
        gap = 20  # Increased gap from circle edge to label for better spacing
        candidates = [
            (cx + r + gap,      cy - H / 2),
            (cx - r - W - gap,  cy - H / 2),
            (cx - W / 2,       cy - r - H - gap),
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, input_folder: str, output_folder: str | None = None,
        ...
        self._output_folder = (Path(output_folder) if output_folder
                               else self._input_folder.parent / f"Feedback_report_{ts}")
...
""")
    ...
    def save_current(self, auto_save: bool = False):
        ...
        try:
            ...
            for i, c in enumerate(self._circles, 1):
                ...
                # ── Anchor dot removed - circle outline is enough ────────────
                # No dot drawing - just the circle outline is sufficient
                ...
                # ── Connector line: from circle centre to label ─────────────
                mid_x = (x1+x2)//2
                mid_y = y2 if ly > cy else y1
                # Line starts from center of circle
                cv2.line(full, (cx, cy), (mid_x, mid_y), col, 2, _AA)
    ...
    def _update_status_bar(self):
        ...
        self._lbl_obj.setText(f"No of Errors: {len(self._circles)}")
